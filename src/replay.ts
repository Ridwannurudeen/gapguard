...
const row = (...c: string[]): string =>
  c.map((s, i) => s.padEnd([18, 9, 8, 12, 13, 11, 10][i])).join("");
console.log(
  row(
    "Time (ET)",
    "Session",
...
for (const tick of scenario) {
  ...
  exposure = rec.risk.targetNotional; // rebalance to target
...
console.log("-".repeat(80));
console.log(
  `Final equity: ${equity.toFixed(2)}  |  Return: ${ret >= 0 ? "+" : ""}${ret.toFixed(2)}%  |  Decisions logged: ${gb.all().length}`,
);
