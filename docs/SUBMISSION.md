...
**Walk-forward RWA alpha certification (`npm run alpha:certify`):** the rule is locked before the OOS pass: follow, not fade, an RWA gap only when the last 80 same-direction RWA gap-follow outcomes have at least 40 observations, positive mean return, and at least 55% wins. The first 60% of unique gap dates are formation history; later dates are out-of-sample. Current result (after fixing a same-date look-ahead leak in the trailing signal, which lowered the return from a stale +3.785%): **116 OOS trades over 16 trading days, +2.62%, win 54.3%, PF 1.73**, versus **−5.471%** for OOS always-fade (`artifacts/rwa-alpha-certification.json`). We deliberately **do not headline an annualized Sharpe** — 16 OOS trading days is far too small for a reliable one, and a trade-frequency-annualized Sharpe overstates because same-day trades across symbols are parallel correlated positions (per-trade Sharpe is 0.20). This is passport alpha evidence on a tiny sample — a hypothesis, not a live edge.
